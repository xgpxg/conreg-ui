...
const configFormat = computed(() => {
  return form.value.configFormat
})
watch(configFormat, () => {
  ...
  formRef.value.validate((ok) => {
    ...
    ElMessageBox.confirm('配置变更将立即生效，确定发布配置？').then(() => {
      R.postJson('/api/config/upsert', {
        namespace_id: namespace.value,
        id: form.value.id + '.' + form.value.configFormat,
    ...
    <div class="mt10">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="80px" size="large">
        <el-form-item label="配置ID" prop="id">
          <el-input v-model="form.id" placeholder="填写配置ID，不包含后缀" maxlength="100" show-word-limit></el-input>
        </el-form-item>
        <el-form-item label="配置格式" prop="configFormat">
          <el-radio-group v-model="form.configFormat">
